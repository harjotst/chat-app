...
router.post('/create', async (req, res) => {
	try {
		...
		await joinActivity.populate({
			path: 'userId',
			select: '-password -rooms',
		});

		res.status(201).json({ msg: 'Room created', activity: joinActivity });
	} catch (error) {
		console.error(error);
...
router.delete('/:roomId', async (req, res) => {
	...
	if (room.createdBy.toString() !== userId) {
		return res.status(403).json({ msg: 'You are not authorized to delete this room' });
	}
...
router.put('/:roomId', async (req, res) => {
	try {
		...
		if (room.createdBy.toString() !== req.session.user.id) {
			return res.status(403).json({ msg: 'Not authorized to update this room' });
		}
...
router.post('/:joinCode/join', async (req, res) => {
	...
	res.status(200).json({ msg: 'Joined the room successfully', activity: joinActivity });
});
...
router.put('/:roomId/update-join-code', async (req, res) => {
	...
	res.status(200).json({ msg: 'Join code updated successfully', joinCode: room.joinCode });
});
...
router.post('/:roomId/kick/:userId', async (req, res) => {
	try {
		...
		await kickActivity.populate({
			path: 'userId',
			select: '-password -rooms',
		});

		res.status(200).json({
			msg: 'User has been kicked out of the room',
...
router.post('/:roomId/leave', async (req, res) => {
	...
	await leaveActivity.populate({
		path: 'userId',
		select: '-password -rooms',
	});

	res.status(200).json({ msg: 'Left the room successfully', activity: leaveActivity });
});
